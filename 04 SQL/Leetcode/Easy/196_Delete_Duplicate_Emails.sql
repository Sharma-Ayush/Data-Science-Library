...
--Solution:
Delete P1 from (Select id, email, RANK() over(partition by email order by id) as [rank]
			from dbo.Person )as P1
where [rank] > 1;
Select id, email
from dbo.Person; 
--or
Delete P1 from dbo.Person as P1
where P1.id <> (Select min(P2.id) from dbo.Person as P2 where P1.email = P2.email);
Select id, email
from dbo.Person;
--or
Delete P1 from dbo.Person as P1
